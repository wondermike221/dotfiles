...
let dark_theme = {
    ...
    date: {|d| 
      ...
      match $i {
        ...
        $i if $i < 6hr => 'orange3',
        $i if $i < 1day => 'yellow3',
        $i if $i < 3day => 'charteuse2b',
        $i if $i < 1wk => 'green3b',
        $i if $i < 6wk => 'darkturquoise',
        $i if $i < 52wk => 'deepskyblue3b',
        _ => 'dark_gray',
      }
    }
    range: white
...
def "clipboard set" [content:string] {
    ^pwsh.exe -NoP -c $"Set-Clipboard -Value ($content)"
}

def "clipboard paste" [ {
    ^powershell.exe -NoP -c 'Get-Clipboard'
}

def "clipboard copy" [content:string] {
    ^powershell.exe -NoP -c $"Set-Clipboard -Value ($content)"
}
